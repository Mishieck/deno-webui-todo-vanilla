...
import { addRefreshableEventListener, AsyncFunction } from "./utils.ts";
...
export type WithExtensions<Ext extends Extensions> =
  & WebuiBridge<Ext>
...
class WebuiBridge<Ext extends Extensions = Extensions> {
      ...
      globalThis.navigation.addEventListener("navigate", (event) => {
        if (!this.#allowNavigation) {
          if (this.#AllEvents && (this.#wsIsConnected())) {
            ...
            if (this.#log) {
              console.log(`WebUI -> DOM -> Navigation Event [${url.href}]`);
            }
      ...
      addRefreshableEventListener(document.body, "a", "click", (event) => {
        if (!this.#allowNavigation) {
          if (this.#AllEvents && (this.#wsIsConnected())) {
            ...
            if (this.#log) {
              console.log(`WebUI -> DOM -> Navigation Click Event [${href}]`);
            }
      ...
      document.addEventListener("mousemove", (e) => {
        ...
        if (!this.#isDragging) {
          ...
          while (target) {
            ...
            let webuiComputed = computedStyle.getPropertyValue(
              "--webui-app-region",
            ).trim();
  ...
  #parseDimensions(
    ...
    try {
      ...
      return [x, y, width, height].some(isNaN)
  ...
  #clicksListener() {
    document.querySelectorAll<HTMLElement>("[id]").forEach((e) => {
      if (
        this.#AllEvents || ((e.id !== "") && (this.#bindsList.includes(e.id)))
      ) {
        ...
        const sendChunk = async () => {
          if (offset < currentPacket.length) {
            const chunkSize = Math.min(
              this.#MULTI_CHUNK_SIZE,
              currentPacket.length - offset,
            );
  ...
  #checkToken() {
    if (this.#wsIsConnected()) {
      ...
      if (this.#log) {
        console.log(
          `WebUI -> Send Token [0x${
            this.#token.toString(16).padStart(8, "0")
          }]`,
        );
      }
    }
        ...
        if (fn.trim()) {
          if (fn !== "__webui_core_api__") {
            if (typeof (window as any)[fn] === "undefined") {
              ...
              (window as any)[fn] = (...args: string[]) =>
                this.call(fn, ...args);
              if (this.#log) {
                console.log(`WebUI -> Binding backend function [${fn}]`);
              }
            }
          }
        }
  ...
  #getScriptUrl() {
    const scripts = Array.from(document.scripts)
      .filter((s) =>
        s.src !== null && s.src.startsWith("http") && s.src.endsWith("webui.js")
      )
      .map((s) => s.src);
  ...
  #wsOnClose = (event: CloseEvent) => {
    if (this.#closeReason === this.#CMD_NAVIGATION) {
      ...
      if (this.#wsStayAlive) {
        ...
        if (this.#log) {
          console.log(
            `WebUI -> Connection lost (${event.code}). Reconnecting...`,
          );
        }
        this.#freezeUi();
    ...
    if (this.#isTextBasedCommand(buffer8[this.#PROTOCOL_CMD])) {
      ...
      switch (buffer8[this.#PROTOCOL_CMD]) {
        ...
        case this.#CMD_JS:
          {
            ...
            // 3: [CMD]
            // 4: [Script]
            const script: string = this.#getDataStrFromPacket(
              buffer8,
              this.#PROTOCOL_DATA,
            );
            ...
            if (this.#log && !FunError) {
              if (isBinaryReturn) {
                ...
                const hexPreview = Array.from(binaryData.slice(0, 64)).map(
                  (b) => `0x${b.toString(16).padStart(2, "0")}`
                ).join(", ");
                console.log(
                  `WebUI -> CMD -> JS -> Return Success (${binaryData.length} Bytes) [${hexPreview}${
                ...
                console.log(
                  `WebUI -> CMD -> JS -> Return Success (${
                    new TextEncoder().encode(stringData).length
                  } Bytes) [${stringData.substring(0, 64)}${
                    stringData.length > 64 ? "..." : ""
                  }]`,
                );
              }
              ...
              console.log(
                `WebUI -> CMD -> JS -> Return Error [${
                  errorString.substring(0, 64)
                }${errorString.length > 64 ? "..." : ""}]`,
              );
            }
            // Protocol
            // 0: [SIGNATURE]
        ...
        case this.#CMD_CALL_FUNC:
          {
            ...
            // 3: [CMD]
            // 4: [Call Response]
            const callResponse: string = this.#getDataStrFromPacket(
              buffer8,
              this.#PROTOCOL_DATA,
            );
            if (this.#log) {
        ...
        case this.#CMD_NAVIGATION:
          ...
          // 3: [CMD]
          // 4: [URL]
          const url: string = this.#getDataStrFromPacket(
            buffer8,
            this.#PROTOCOL_DATA,
          );
        ...
        case this.#CMD_WINDOW_RESIZED:
          ...
          // 3: [CMD]
          // 4: [x,y,width,height]
          const widthAndHeight: string = this.#getDataStrFromPacket(
            buffer8,
            this.#PROTOCOL_DATA,
          );
          ...
          if (this.#log) {
            console.log(
              `WebUI -> CMD -> Window Resized [x: ${x}, y: ${y}, width: ${width}, height: ${height}]`,
            );
          }
          break;
        case this.#CMD_NEW_ID:
          ...
          // 3: [CMD]
          // 4: [New Element]
          const newElement: string = this.#getDataStrFromPacket(
            buffer8,
            this.#PROTOCOL_DATA,
          );
          if (this.#log) {
          ...
          if (!this.#bindsList.includes(newElement)) {
        ...
        case this.#CMD_CHECK_TK:
          ...
          if (status) {
            if (this.#log) {
              console.log(`WebUI -> CMD -> Token [${tokenHex}] Accepted`);
            }
            this.#TokenAccepted = true;
            // Get binds list (CSV)
            let csv: string = this.#getDataStrFromPacket(
              buffer8,
              this.#PROTOCOL_DATA + 1,
            );
            ...
            if (this.#log) {
              console.log(
                `WebUI -> CMD -> Token [${tokenHex}] Not Accepted. Reload page...`,
              );
            }
      ...
      switch (buffer8[this.#PROTOCOL_CMD]) {
        case this.#CMD_SEND_RAW:
          ...
          // Get function name
          const functionName: string = this.#getDataStrFromPacket(
            buffer8,
            this.#PROTOCOL_DATA,
          );
          // Get the raw data
          const rawDataIndex: number = this.#PROTOCOL_DATA +
            functionName.length + 1;
          const rawDataSize: number = (buffer8.length - rawDataIndex) - 1;
          const userRawData = buffer8.subarray(
            rawDataIndex,
            rawDataIndex + rawDataSize,
          );
          if (this.#log) {
            console.log(
              `WebUI -> CMD -> Received Raw ${rawDataSize} bytes for [${functionName}()]`,
            );
          }
    ...
    if (fn !== "__webui_core_api__") {
      if (!this.#AllEvents && !this.#bindsList.includes(`${fn}`)) {
        return Promise.reject(
          new ReferenceError(`No binding was found for "${fn}"`),
        );
      }
    }
